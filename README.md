...
## 🛠️ Technical Stack
- **Framework:** Ultralytics YOLOv8 (Nano variant for Edge AI)
- **Language:** Python 3.10+
- **Libraries:** OpenCV (Image Processing), Pandas (Data Engineering), Matplotlib (Visualization)
- **Environment:** Linux (Ubuntu/Debian) via Virtual Environments (venv)
...
1. **Snapshot Serengeti (Direct S3):** Restricted access due to authentication gatekeeping (HTTP 403 Forbidden).
2. **DataDryad Repository:** Encountered link rot and deprecated file paths (HTTP 404 Not Found).
3. **LILA Science (Blob Storage):** Encountered permission-based connection errors and authorization blocks.
...
I developed a pre-processing script (`prepare_labels.py`) to transform raw pixel coordinates into a scale-invariant format. For an image with width $W$ and height $H$, and a bounding box defined by $(x_{min}, y_{min}, width, height)$, the math is as follows:

1.  **Center Calculation:**
    - $x_{center} = (x_{min} + (width / 2)) / W$
    - $y_{center} = (y_{min} + (height / 2)) / H$
